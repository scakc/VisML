function init(nClasses, nSamples, seed){
    ...
    for (var i = 0; i < nClasses; i++) {
        ...
        datay = tf.add(datax ,tf.randomNormal(datax.shape,0, 0.25));
...
function process(){
    ...
    lam = parseInt(document.getElementById("lam").value);
    
    CS = datas.shape[0];

    var r_vals = datas.slice([0,1], [CS,1]).reshape([-1]); // X
    var r_data = datas.slice([0,0], [CS,1]).reshape([-1]); // Y
    var r_grid = grid;
    
    N = r_grid.shape[0];

    var X = r_data.reshape([-1,1]);
    var Y = r_vals.reshape([-1,1]);
    var Xstr = r_grid.reshape([-1,1]);
    var D = X.shape[0];

    var inv_xtx = tf.div(1,tf.add(tf.dot(tf.transpose(X), X),lam));
    var xty = tf.dot(tf.transpose(X), Y)
    var W = tf.dot(inv_xtx, xty);

    labels = tf.mul(W, Xstr);
...
document.getElementById("cfy").onclick = function() {process()};
